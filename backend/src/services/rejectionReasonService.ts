...
export class RejectionReasonService {
   ...
   */
  async generateRejectionReason(input: RejectionReasonInput): Promise<string> {
    try {
      // If LLM is disabled or API key not set, use fallback
      if (process.env.ANTHROPIC_API_KEY && process.env.USE_LLM_REJECTION === 'true') {
        try {
          const reason = await llmService.generateRejectionReason({
   ...
   */
  private getFallbackRejectionReason(input: RejectionReasonInput): string {
    const templates: Record<string, string> = {
      invalid_document:
        `Dear ${input.fullName}, Your KYC application has been rejected due to invalid or unverifiable identification document. Please resubmit with a valid ${input.idType} that is clearly legible and within the validity period. If you have questions, please contact our support team.`,
...
export const rejectionReasonService = new RejectionReasonService();
